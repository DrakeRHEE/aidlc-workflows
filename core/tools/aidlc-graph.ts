...
/** The section that carries a record's EXECUTE/SKIP grid. A record is exactly
 *  the harness scope `.md` plus this one appended section, so projecting a record
 *  into the harness tree is a pure strip and back-filling a record from an
...
const COMPOSED_GRID_HEADING = "## Stage Grid";

export interface ComposedScopeRecord {
  /** The scope name from frontmatter (the grid key and `--scope` argument). */
  name: string;
  /** The harness-tree projection: everything above the Stage Grid section. */
  identity: string;
...
export function parseComposedScopeRecord(
  ...
  const headingAt = body.indexOf(`\n${COMPOSED_GRID_HEADING}`);
  if (headingAt === -1) {
    throw new Error(
      `Composed scope record ${filePath} has no "${COMPOSED_GRID_HEADING}" section. ` +
        `Append one holding a \`\`\`json fence with {"stages": {...}}.`,
    );
  }
  const section = body.slice(headingAt);
  const fence = /```json\s*\n([\s\S]*?)```/.exec(section);
  if (fence === null) {
    throw new Error(
      `Composed scope record ${filePath} has a "${COMPOSED_GRID_HEADING}" section with no ` +
        "```json fence.",
    );
  }
  let parsed: unknown;
  try {
    ...
    throw new Error(
      `Composed scope record ${filePath} has an unparseable Stage Grid fence: ${errorMessage(err)}`,
    );
  }
  const stagesRaw = (parsed as { stages?: unknown } | null)?.stages;
  if (typeof stagesRaw !== "object" || stagesRaw === null || Array.isArray(stagesRaw)) {
    throw new Error(
      `Composed scope record ${filePath}: the Stage Grid fence must be an object with a ` +
  ...
  return { name, identity: body.slice(0, headingAt + 1).replace(/\n+$/, "\n"), stages };
}
 ...
 *  round-trip through parse/render is byte-stable. */
