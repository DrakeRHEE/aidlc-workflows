...
| Composed scope durability | Every composer-authored scope's durable `aidlc/scopes/<name>.md` record and its harness projection agree. Fails on a scope file with no grid column (it would resolve as an empty all-SKIP plan), a record not yet projected into the harness tree, or a workflow whose recorded `Scope` has no resolvable definition. Names `aidlc-graph.ts compile` as the remedy |
