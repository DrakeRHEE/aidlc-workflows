...
| | Path | Role |
|---|---|---|
| **Record** | `aidlc/scopes/<name>.md` | The durable source. Identity frontmatter plus a `## Stage Grid` section holding the EXECUTE/SKIP plan, in one file. Commit it with the rest of your `aidlc/` tree |
...
If you have a composed scope from an earlier release it lives only in the harness tree; the first `graph compile` after upgrading writes its record for you, once. From then on the record is the source. `/aidlc --doctor`'s **Composed scope durability** check reports any scope whose record and projection have drifted apart.
